...
echo T_("<p>En effet, seulement deux fichiers doivent être inclus dans chaque page (un au début et un à la fin de chaque page), et ces derniers fournissent une structure de site optimisée pour le référencement et traduite dans la langue de la page (si la langue est disponible): chaque page peut avoir facilement sa propre balise <code>title</code>, ses propres métabalises, son titre de premier niveau (<code>h1</code>), sa propre langue, etc. L'en-tête personnalisée permet d'éviter le contenu dupliqué dans les moteurs de recherche, d'avoir un site optimisé pour le référencement et d'offrir des repères aux internautes.</p>");

echo T_("<p>Aussi, un fichier de configuration permet de choisir par simple renseignement de variables le nombre et l'emplacement des colonnes, l'emplacement des menus dans le flux HTML, la présence ou non de certaines structures comme le bas de page, etc.</p>");

echo T_("<p>Enfin, Squeletml fournit quelques modules prêts à l'emploi, comme des formulaires de contact, des galeries photo, une interface d'administration reproduisant sensiblement les principales actions normalement effectuées par ftp, etc. Le tout peut facilement être traduit puisque Squeletml utilise PHP Gettext pour l'affichage de l'interface.</p>");
