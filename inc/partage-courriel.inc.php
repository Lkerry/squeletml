...
if (!$erreur404 && !$estPageDerreur)
{
	...
	elseif (empty($courrielContact))
	{
		...
		if (isset($_GET['action']) && $_GET['action'] == 'partageCourriel')
		{
			...
			{
				$messagePartageCourrielSupplement = publicationsRecentes($racine, $urlRacine, eval(LANGUE), 'categorie', $idCategorie, 5, FALSE, FALSE, $datePublicationVautDateRevision, $galerieFluxRssAuteurEstAuteurParDefaut, $auteurParDefaut, $galerieLegendeMarkdown, $galerieLienOriginalTelecharger, $marqueTroncatureApercu, $dureeCache, $estPageCron, $mettreAjourCacheSeulementParCron);
