...
<input type="radio" name="etat" value="horsLigne" <?php if (adminSiteEnMaintenance($racine . '/.htaccess')) {echo 'checked="checked"';} ?> /> <?php echo T_("en maintenance (hors ligne)."); ?> <?php
if (adminSiteEnMaintenance($racine . '/.htaccess'))
{
	if ($ip = adminSiteEnMaintenanceIp($racine . '/.htaccess'))
	{
		echo sprintf(T_("L'IP %1\$s a accès au site hors ligne."), $ip);
	}
	else
	{
		echo T_("Aucune IP n'a accès au site hors ligne.");
	}
}
?>
</p>
...
<?php
$ip = adminSiteEnMaintenanceIp($racine . '/.htaccess');
if ($ip)
{
	$valeurChampIp = $ip;
}
